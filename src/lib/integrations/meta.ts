...
export class InstagramPublisher implements SocialPublisher {
  ...
  async publish(request: PublishRequest): Promise<PublishResult> {
    ...
    const createUrl = new URL(`https://graph.facebook.com/${this.graphVersion}/${this.instagramAccountId}/media`);
    createUrl.searchParams.set("access_token", request.accessToken);
    ...
    const containerResponse = await fetch(createUrl, { method: "POST", signal: AbortSignal.timeout(20_000) });
    ...
    const publishUrl = new URL(`https://graph.facebook.com/${this.graphVersion}/${this.instagramAccountId}/media_publish`);
    publishUrl.searchParams.set("access_token", request.accessToken);
    publishUrl.searchParams.set("creation_id", container.id);
    const publishResponse = await fetch(publishUrl, { method: "POST", signal: AbortSignal.timeout(20_000) });
